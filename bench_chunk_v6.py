...
def main(model_path: str, chunk_size: int, save_results: str = None, save_plot: str = None):
    ...
    # 5个 ~1000 tokens，接着 1个 ~5000 tokens，再接着 4个 ~1000 tokens
    long_prompts = (
        [[random.randint(0, vocab_size - 1) for _ in range(random.randint(980, 1020))] for _ in range(5)] +
        [[random.randint(0, vocab_size - 1) for _ in range(random.randint(4800, 5200))]] +
        [[random.randint(0, vocab_size - 1) for _ in range(random.randint(980, 1020))] for _ in range(4)]
    )
